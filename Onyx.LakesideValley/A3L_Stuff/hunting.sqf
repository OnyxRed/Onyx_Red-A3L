    ...
    animalMeat =
    [
            ["Goat_random_F", "Goat",2,2,"raw goat meat"],
...
fnc_gutanimal = {
	...
	if (!_iskilled) exitwith {["Animal is not dead yet!",5,"red"] call A3L_Fnc_Msg; life_action_inUse = false;};
	_animalname = "";
	_amountyouget = 0;
	_extraamount = 0;
	_itemyouget = "";
	...
	[_fancytexty,5,"red"] call A3L_Fnc_Msg;
	life_action_inUse = false;	
	};
	_guttext = format ["Gutting %1, please wait.",_animalname];
	[_guttext,5,"blue"] call A3L_Fnc_Msg;
	player playMoveNow "AinvPknlMstpSnonWnonDnon_medic_1";
	sleep 3;
	waitUntil{animationState player != "AinvPknlMstpSnonWnonDnon_medic_1";};
	_extraamount = round (random _extraamount);
	_totalamount = _amountyouget + _extraamount;
	if (!([true,_itemyouget,_totalamount] call life_fnc_handleInv)) exitwith {["Your inventory is full!",5,"red"] call A3L_Fnc_Msg; life_action_inUse = false;};
	deletevehicle _cursortarget;
	_gotitem = format ["You got yourself %1 %2",_totalamount,_itemyouget];
	[_gotitem,10,"green"] call A3L_Fnc_Msg;
	life_action_inUse = false;
};