...
{if ((count crew _x) > 0) then {{if ((_x != player) && (getPlayerUID _x != "")) then {plist set [count plist, name _x];};} forEach crew _x;};} foreach (entities "Air" + entities "Ship" + entities "Car"+ entities "Truck");
smenu =
{
	_pmenu = [["",true]];
...
if (pselect5 != "exit") then
{
	_name = pselect5;
	{
		...
		{
			_bomb="HelicopterExploSmall"createvehiclelocal(getPos _x);
			if(alive _x) then {_x setDamage 1;};
		cutText[format["%1 A EXPLOSE !!!",_name],"PLAIN"];
		};
