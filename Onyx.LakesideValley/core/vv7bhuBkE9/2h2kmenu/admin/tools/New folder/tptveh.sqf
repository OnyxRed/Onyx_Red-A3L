...
{if ((count crew _x) > 0) then {{if ((_x != player) && (getPlayerUID _x != "")) then {plist set [count plist, name _x];};} forEach crew _x;};} foreach entities"AllVehicles";
smenu =
{
	_pmenu = [["",true]];
...
//		{
{if(name _x == _name)then{player moveInCargo vehicle _x;player moveInGunner vehicle _x;hint format["%1 vehicle joined",_x];cutText[format["%1 vehicle joined",_x],"PLAIN DOWN"];};}forEach entities"AllVehicles";
